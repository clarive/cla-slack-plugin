...
reg.register("service.slack.post_message", {
    ...
    handler: function(ctx, config) {
        ...
        try {
            ...
            log.fatal(
                "Could not find Slack Webhook resource=" + config.webhook
            );
            return;
        }

        var url =
            webhookResource.webhookURL() ||
            log.fatal(
                _("Missing URL for Slack Webhook resource: " + config.webhook)
            );
        ...
        if (typeof config.payload == "object") {
            Object.keys(config.payload).forEach(function(key) {
                payload[key] = config.payload[key];
            });
        } else if (config.jsonData) {
            var jsonData = JSON.parse(config.jsonData);
            if (typeof jsonData == "object") {
                Object.keys(jsonData).forEach(function(key) {
                    payload[key] = jsonData[key];
                });
            } else {
        ...
        try {
            ...
            log.fatal(
                _(
                    "Could not parse JSON data for Slack message payload: " +
                        payload
                )
            );
        }
        ...
        var res = ag.postForm(url, form, options);
    }
});
